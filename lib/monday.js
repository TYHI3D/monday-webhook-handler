...
const { 
  ...
  async function createProjectIntakeItem(formData, contactId, webFormItemId) {
    ...
    const values = {
      // Link to Contact
      [PROJECT_INTAKE_COLUMNS.CONTACTS]: { item_ids: [parseInt(contactId)] }
    };
    
    // Add optional fields if they exist
    if (deadline) values[PROJECT_INTAKE_COLUMNS.DEADLINE] = { date: deadline };
    if (projectType && projectType.length) values[PROJECT_INTAKE_COLUMNS.PROJECT_TYPE] = { labels: projectType };
    if (materials) values[PROJECT_INTAKE_COLUMNS.MATERIAL] = materials;
    if (detailsText) values[PROJECT_INTAKE_COLUMNS.DETAILS] = { text: detailsText };
    if (scanNeeds) values[PROJECT_INTAKE_COLUMNS.SCAN_NEEDS] = { labels: [scanNeeds] };
    if (canBringObject) values[PROJECT_INTAKE_COLUMNS.CAN_BRING_OBJECT] = canBringObject;
    if (partQuantity) values[PROJECT_INTAKE_COLUMNS.PART_QUANTITY] = partQuantity;
    if (partDimensions) values[PROJECT_INTAKE_COLUMNS.PART_DIMENSIONS] = partDimensions;
    if (projectFiles) values[PROJECT_INTAKE_COLUMNS.FILES] = projectFiles;
    
    // Set intake source to Web Form
    values[PROJECT_INTAKE_COLUMNS.INTAKE_SOURCE] = { label: "Web Form" };
    
    // Add today's date as received date
    const today = new Date().toISOString().split('T')[0];
    values[PROJECT_INTAKE_COLUMNS.DATE_RECEIVED] = { date: today };
    
    // Link back to the web form item
    if (webFormItemId) {
      values[PROJECT_INTAKE_COLUMNS.WEB_FORM_RAW_INTAKE] = { item_ids: [parseInt(webFormItemId)] };
    }
    ...
    const data = await runGraphQLQuery(mutation);
    const projectId = data?.data?.create_item?.id;
