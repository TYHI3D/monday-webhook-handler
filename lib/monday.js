...
const { 
  ...
  async function createProjectIntakeItem(formData, contactId, webFormItemId) {
    ...
    try {
      ...
      // Update dropdown (project type)
      if (projectType && projectType.length > 0) {
        updates.push(updateColumnValue(
          BOARD_IDS.PROJECT_INTAKE,
          projectId,
          PROJECT_INTAKE_COLUMNS.PROJECT_TYPE,
      ...
      // Update dropdown (scan needs)
      if (scanNeeds) {
        const scanValue = Array.isArray(scanNeeds) ? scanNeeds[0] : scanNeeds;
        updates.push(updateColumnValue(
          BOARD_IDS.PROJECT_INTAKE,
          projectId,
          PROJECT_INTAKE_COLUMNS.SCAN_NEEDS,
          { labels: [scanValue] }
        ));
      }
      
      // Update status (can bring object)
      if (canBringObject) {
        updates.push(updateColumnValue(
          BOARD_IDS.PROJECT_INTAKE,
          projectId,
          PROJECT_INTAKE_COLUMNS.CAN_BRING_OBJECT,
      ...
      // Update status (intake source)
      updates.push(updateColumnValue(
        BOARD_IDS.PROJECT_INTAKE,
        projectId,
        PROJECT_INTAKE_COLUMNS.INTAKE_SOURCE,
        { label: "Web Form" }
      ));
