    ...
    try {
      ...
      try {
        if (existingRaw) {
          ...
          // Extract existing project IDs from the linkedPulseIds array
          if (parsed && parsed.linkedPulseIds && Array.isArray(parsed.linkedPulseIds)) {
            existingIds = parsed.linkedPulseIds.map(item => item.linkedPulseId);
      ...
      // Format the array as expected by Monday.com API - as item_ids array
      const mutation = `
        mutation {
          change_column_value(
            board_id: ${BOARD_IDS.CONTACTS},
